...
myLayout = myLayoutModifiers $ layoutTall ||| layoutWide ||| layoutSpiral  ||| layoutCenter ||| layoutFull
    where
        layoutFull = Full
        layoutCenter = ThreeColMid 1 (3/100) (1/2)
        layoutSpiral = spiral (6/7)
        layoutWide = Mirror layoutTall
        layoutTall = Tall nmaster delta tiled_ratio
          where
            nmaster = 1
            delta = 3/100
            tiled_ratio = 1/2

myLayoutModifiers = myBorders . avoidStruts . mkToggle (NBFULL ?? NOBORDERS ?? EOT)
    where
        -- Customizable screen & window borders
        myBorders = spacingRaw True               -- Only borders for >1 windows
                               (Border 0 2 2 2)   -- screen border (Border top bottom right left)
                               True               -- enable screen edge borders
                               (Border 2 2 2 2)   -- window borders
...
, layoutHook = gaps [(U,33), (D,3), (R,3), (L,3)] $ myLayout ||| layoutHook myBaseConfig
